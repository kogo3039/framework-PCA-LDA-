...
class Classifier:
    ...
    def __init__(self, *args):
        self.args = list(args)
        self.total = np.sum([a.shape[0] for a in self.args])
        ...
        if self.args[0].shape[1] == 1:
            ...
            for i in range(len(self.args)):
                g = -(1/2)*(x-self.means[i]) * self.invs[i] * (x-self.means[i]).T\
                    -(1/2) * np.log(np.abs(self.covs[i])) + np.log(len(self.args[i])/self.total+1e-6)
...
class PCA:
    def __init__(self, train0, test0, train1, test1, train2, test2):
        self.train0 = train0
        self.train1 = train1
        self.train2 = train2
        self.test0 = test0
        ...
        mu_x2 = np.mean(train2, axis=0)
               

    def __call__(self, interval=2):
        
        train = np.concatenate((self.train0, self.train1, self.train2), axis=0)
        cov = np.cov(train.T)
        ...
        trn2 = np.array((self.train2 @ self.eVec[:, :interval]), ndmin=2)
        tst0 = np.array((self.test0 @ self.eVec[:, :interval]), ndmin=2)
        tst1 = np.array((self.test1 @ self.eVec[:, :interval]), ndmin=2)
        tst2 = np.array((self.test2 @ self.eVec[:, :interval]), ndmin=2)
        
        return trn0, tst0, trn1, tst1, trn2, tst2
...
class LDA:
    def __init__(self, train0, test0, train1, test1, train2, test2):
        ...
        cov_x2 = np.cov(train2.T)
        mu_x0 = np.mean(train0, axis=0)
        mu_x1 = np.mean(train1, axis=0)
        mu_x2 = np.mean(train2, axis=0)
        mu = (mu_x0 + mu_x1 + mu_x2) / 3
        self.Sw = cov_x0 + cov_x1 + cov_x2
        self.Sw_inv= np.linalg.inv(self.Sw)
        self.Sb = np.array(mu_x0 - mu, ndmin=2).T @ \
                  np.array(mu_x0 - mu, ndmin=2) + \
                  np.array(mu_x1 - mu, ndmin=2).T @ \
                  np.array(mu_x1 - mu, ndmin=2) + \
                  np.array(mu_x2 - mu, ndmin=2).T @\
                  np.array(mu_x2 - mu, ndmin=2)
